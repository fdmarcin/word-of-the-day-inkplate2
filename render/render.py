...
def fetch_wordlist(url: str) -> list[str]:
    try:
        ...
        print(f"Failed to fetch word list from {url}: {e}")
        sys.exit(1)

    if resp.status_code != 200:
        print(f"Failed to fetch word list from {url}: HTTP {resp.status_code}")
        sys.exit(1)

    return parse_wordlist(resp.text)


def load_words() -> list[str]:
    wordlist_url = os.environ.get(WORDLIST_URL_ENV)
    if wordlist_url:
        print(f"Fetching word list from {wordlist_url}")
        return fetch_wordlist(wordlist_url)

    return load_json(WORDS_JSON, [])


def reset_history_if_exhausted(words: list[str], history: set[str]) -> set[str]:
    if set(words).issubset(history):
        print("All words have been shown. Clearing history.json and starting over.")
        history = set()
...
def main():
    ...
    if not words:
        if os.environ.get(WORDLIST_URL_ENV):
            print("Remote word list is empty.")
        else:
            print("words.json is empty. Run parse_wordlist.py first.")
        sys.exit(1)

    history = reset_history_if_exhausted(words, history)
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if word is None:
            ...
            word = pick_word(words, history)
            if word is None:
                print("No words available after clearing history.json.")
                sys.exit(1)
