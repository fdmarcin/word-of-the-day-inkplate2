...
You can adapt this to [use another screen](#for-another-board-or-screen) or learn a [different language](#for-another-language).
...
To use a remote word list that is fetched before each new word is picked, set `WORDLIST_URL`:

```sh
WORDLIST_URL="https://example.com/wordlist.txt" python main.py
```

The remote file must have one word or phrase per line. When every word in the current list is already in `data/history.json`, the script clears `data/history.json` and starts over.
...
The word used is added to `data/history.json` so it doesn't appear again until the current word list has been exhausted.
